...
export async function createOrUpdateRating(
	...
	const q = query(ratingsRef, where('userId', '==', data.userId), where('houseId', '==', data.houseId));
...
export async function getRatingsForHouse(familyId: string, houseId: string): Promise<Rating[]> {
	const db = getFirestoreInstance();
...
export async function getRatingByUserAndHouse(
	...
	const db = getFirestoreInstance();
	const ratingsRef = collection(db, 'families', familyId, 'ratings');
	const q = query(ratingsRef, where('userId', '==', userId), where('houseId', '==', houseId));
	const snapshot = await getDocs(q);
