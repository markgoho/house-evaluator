...
function calculatePricePerSqFt(price: number | null, squareFeet: number | null): number | null {
...
export async function getHouse(familyId: string, houseId: string): Promise<House | null> {
	const db = getFirestoreInstance();
...
export async function deleteHouse(familyId: string, houseId: string): Promise<void> {
	const db = getFirestoreInstance();
	const houseRef = doc(db, 'families', familyId, 'houses', houseId);
